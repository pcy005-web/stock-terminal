...
app = Flask(__name__)
application = app  # 배포 환경(Gunicorn, Vercel 등) 호환용 WSGI 엔트리포인트
...
def fetch_feature_stocks():
  ...
  query = (
      '코스피 마감 특징주 when:6h' if is_market_closed else '[특징주] 급등 when:6h'
  )
      ...
      for item in root.findall('.//item'):
        ...
        if not raw_stock_name:
          ...
          for qm in quoted_matches:
            if (
                len(qm) <= 12
                and not any(ew in qm for ew in exclude_words)
                and not any(char.isdigit() for char in qm)
            ):
              raw_stock_name = qm
              break
  ...
  fallbacks = [
      {
          ...
          'title': (
              f'[{current_time_str}] [특징주] 버크셔 해서웨이 포트폴리오 조정 및'
              ' 시장 영향 분석'
          ),
      ...
      },
      {
          ...
          'title': (
              f'[{current_time_str}] [일본 특징주] MOL, 중동발 선박가 급등에'
              ' 노후 유조선 매각 검토'
          ),
          'link': 'https://news.google.com',
          'timestamp': now_dt,
      ...
      },
      {
          ...
          'title': (
              f'[{current_time_str}] [ET특징주] 앤씨앤, 비투엔에 피인수... 주가 上'
          ),
          'link': 'https://news.google.com',
          'timestamp': now_dt,
          'raw_title': '앤씨앤 피인수',
      ...
      {
          'stock_full': '미투온 - 게임/콘텐츠',
          'title': (
              f"[{current_time_str}] [ET특징주] '카카오게임즈 피인수' 미투온,"
              ' 상한가 이어 19%↑'
          ),
          'link': 'https://news.google.com',
          'timestamp': now_dt,
      ...
      },
      {
          'stock_full': '한화시스템 - 방산',
          'title': (
              f'[{current_time_str}] [특징주] 한화시스템, 방산 수출 확대 기대감에'
              ' 강세'
          ),
          'link': 'https://news.google.com',
          'timestamp': now_dt,
  ...
  if is_market_closed:
    market_summary_keyword = (
        ...
        '• [주도 업종 섹터]: 반도체 대형주(삼성전자, SK하이닉스 등) 및 핵심 주도주'
        ' 반등 주도\n'
...
def fetch_naver_finance_news():
    ...
    with urllib.request.urlopen(
      ...
      for item in root.findall('.//item'):
        ...
        for m in quoted_matches:
          if (
              len(m) > 6
              or any(char.isdigit() for char in m)
              or any(ew in m for ew in exclude_words)
          ):
            continue
        ...
        if extracted_stocks_from_quotes:
          ...
          if is_negative:
            related_stock = '원/달러 환율, 지수 방어주'
              ...
              k in title_clean for k in ['반도체', 'AI', '삼성', '하이닉스', '엔비디아']
          ):
  ...
  if len(news_list) < 10:
    dynamic_fallbacks = [
        (
            f'[{current_hour_str} 전문가 리포트] 글로벌 공급망 재편에 따른 반도체'
            ' 핵심 소부장 펀더멘털 분석',
            'https://news.google.com',
            '삼성전자 - AI 반도체',
            '실적 추정치 상향 조정 기업 중심의 밸류에이션 매력 점검',
            '호재',
            False,
        ),
        (
            f'[{current_hour_str} 매크로 검증] 환율 변동성 확대에 따른 수출주'
            ' 컨센서스 영향 진단',
            'https://news.google.com',
            '현대차 - 자동차',
            '외국인 수급 민감도에 연동된 환차익 및 마진율 변화 모니터링',
            '중립',
            False,
        ),
        (
            f'[{current_hour_str} 기업공시 분석] 주요 상장사 실적 가이던스 및'
            ' 주주환원 정책 적정성 평가',
            'https://news.google.com',
            'KB금융 - 금융',
            '자기자본이익률(ROE) 개선세 기반의 하방 경직성 확보',
            '호재',
            False,
        ),
        (
            f'[{current_hour_str} 수급 포커스] K-방산 수출 다변화 및 수주 잔고'
            ' 기반 실적 가시성 분석',
            'https://news.google.com',
            '한화에어로스페이스 - 방산',
            '중장기 실적 성장이 담보된 수주형 성장주 트레이딩',
            '호재',
            False,
        ),
        (
            f'[{current_hour_str} 리스크 점검] 미국 국채 금리 경로 불확실성에'
            ' 따른 성장주 멀티플 압박 요인',
            'https://news.google.com',
            '미국 국채 - 매크로',
            '할인율 상승에 따른 밸류에이션 부담 완충 여부 검증',
            '리스크',
            True,
        ),
    ]
...
def generate_theme_sync_analysis(quotes, news_list):
  ...
  if is_up:
    us_driver = (
        f'글로벌 빅테크 반도체 밸류체인 연동 강세: 필라델피아 반도체({sox_rate}) 및'
        f' 나스닥 선물({nasdaq_rate})의 우상향 흐름은 국내 반도체 수출 실적 개선'
        ' 기대감을 선반영하며 지수 상단을 지지하고 있습니다.'
    )
    core_stocks = 'NVIDIA, 마이크론 테크놀로지, ASML'
    domestic_stocks = '삼성전자 - AI 반도체, SK하이닉스 - AI 반도체'
    risk_strategy = (
        '실적 모멘텀이 검증된 펀더멘털 우량주 중심의 공격적 비중 확대 및 눌림목'
        ' 트레이딩'
    )
  else:
    us_driver = (
        f'글로벌 기술주 멀티플 조정 압력: 필라델피아 반도체({sox_rate}) 조정 및'
        f' 나스닥 선물({nasdaq_rate})의 경계감 반영은 국내 증시의 단기'
        ' 변동성을 확대시키는 주요 요인으로 작용합니다.'
    )
...
def generate_smart_money_analysis(quotes):
  ...
  badge_text = (
      '외인·기관 주도세력 순매수 유입 (포지션 확장)'
      if kospi_up
      else '외인·기관 주도세력 매도 우위 (방어적 포지션)'
  )
  badge_class = 'up' if kospi_up else 'down'

  domestic_text = (
      f"국내 현·선물 수급 동향: 코스피({kospi.get('rate')}),"
      f' 코스닥({kosdaq.get(\'rate\')})의 방향성과 연동하여 주도세력의 누적'
      ' 순매수를 모니터링합니다.'
  )
  decoupling_text = (
      '코스피 대형주와 코스닥 개별주 간의 차별화 장세가 전개되는 가운데, 지수'
      ' 방어력을 갖춘 핵심 주도주와 실적 개선 개별 종목 간의 빠른 순환매 수급'
      ' 포착'
  )
  concentrated_themes = (
      '<strong>현재 스마트머니 수급 집중 테마 및 업종 분석:</strong>'
      ' 1) <strong>AI 반도체 대형주(삼성전자, SK하이닉스)</strong> 중심의 이익'
      ' 성장 동반 구조적 쏠림 현상이 지속되고 있으며,'
      ' 2) 변동성 장세 속 수익성 방어를 위한'
      ' <strong>전력기기·원전·조선</strong> 및 <strong>은행·보험 등 저PBR'
      ' 주주환원 업종</strong>으로 자금이 분산·확산되는 순환매 흐름이 포착됩니다.'
  )
  fx_oil_text = (
      f"원/달러 환율({usdkrw.get('price')}원) 변동성에 따른 외국인 수급"
      ' 민감도 점검'
  )
  ...
  return [
      {
          ...
          'stock': (
              '삼성전자 - AI 반도체, SK하이닉스 - AI 반도체, 한미반도체 - AI'
              ' 반도체'
          ),
          'rank': 'TOP 1',
      },
...
def generate_premarket_summary_bullets(quotes, news_list):
  nasdaq_fut = quotes.get(
      'nasdaq_fut', {'price': '-', 'rate': '-0.6%', 'is_up': False}
  )
  usdkrw = quotes.get(
      'usdkrw', {'price': '1,300', 'rate': '+0.00%', 'is_up': True}
  )
  ...
  return [
      (
          '9월 FOMC 금리 인상 단행 및 매파적 여진: 연준의 스탠스로 인해 단기'
          ' 변동성 확대 압력이 가중되고 있으나 장기물 금리의 상승 속도를'
          ' 주시해야 합니다.'
      ),
      (
          '금리 인상 사이클과 증시 영향: 금리 인상 그 자체를 추세 하락으로'
          ' 해석하기보다는, 당시의 경기 및 이익 사이클과 맞물린 장기물 금리의'
          ' 상승 폭이 핵심 관전 포인트입니다.'
      ),
      (
          f"해외 지표 및 환율 동향: 나스닥 선물({nasdaq_fut.get('rate')})과"
          f" 필라델피아 반도체 지수({sox.get('rate')}) 등락 속"
          f" 원/달러 환율({usdkrw.get('price')}원)의 변동성을 점검합니다."
      ),
      (
          '대응 전략: FOMC 직후 단기 변동성은 매수 기회로 활용하되, AI 반도체'
          ' 및 주주환원 우수 업종 중심의 실적 모멘텀을 선별 기준으로 삼는 것이'
          ' 적절합니다.'
      ),
  ]
...
def generate_ai_comprehensive_briefing(quotes, news_list):
  ...
  return (
      ...
      f'📊 [시황 총평]\n'
      f"나스닥 선물({nasdaq_fut['rate']})과 필라델피아 반도체 지수({sox['rate']})"
      ' 변동성을 소화하며 대형주 중심의 완만한 수급 균형이 나타나고 있습니다.'
      f" 원/달러 환율({usdkrw['price']}원) 추이에 주목합니다.\n\n"
      f'🔍 [핵심 체크포인트]\n'
      f'• 주요 헤드라인: "{top_news}"\n'
...
def index():
  ...
  market_summary_bullets = generate_premarket_summary_bullets(
      price_map, live_news
  )
...
def api_feature_stocks():
  ...
  return json.dumps(
      {'feature_stocks': items, 'feature_market_summary': market_summary},
      ensure_ascii=False,
  )
