...
def fetch_realtime_data(ticker):
    ...
    try:
        ...
        # 3. 원자재(WTI 원유) API
            ...
            with urllib.request.urlopen(req, context=get_ssl_context(), timeout=5) as response:
                res_json = json.loads(response.read().decode('utf-8'))
                # 구조에 따라 필드명 확인 (일반적으로 closePrice, fluctuationsRatio 등 포함)
        ...
        # 5. 원/달러 환율 API
            ...
            with urllib.request.urlopen(req, context=get_ssl_context(), timeout=5) as response:
                res_json = json.loads(response.read().decode('utf-8'))
                # 환율 API의 리스트/데이터 구조에 맞게 첫 번째 값 추출
                list_data = res_json.get('list', []) or res_json.get('datas', [])
                if list_data:
                    item = list_data[0]
                    cur_price = item.get('dealBasRate') or item.get('closePrice') or item.get('nowValue')
                    fluc_rate = item.get('fluctuationsRatio') or item.get('rate')
                    sign = item.get('sign')
                    if cur_price:
                        price_val = float(str(cur_price).replace(',', ''))
