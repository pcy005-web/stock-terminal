    ...
    },
    {
        ...
        'stocks': [
            ...
            {'code': 'nasdaq', 'name': '나스닥', 'ticker': 'NAVER_WORLD_SPOT_NAS'},
            {
                'code': 'sp500_fut',
                'name': 'S&P 500 선물',
                'ticker': 'NAVER_WORLD_ES',
            },
            {
                'code': 'dow_fut',
                'name': '다우존스 선물',
                'ticker': 'NAVER_WORLD_YM',
            },
            {
                'code': 'nasdaq_fut',
                'name': '나스닥 선물',
                'ticker': 'NAVER_WORLD_NQ',
            },
            {
                'code': 'phlx',
                'name': '필라델피아 반도체',
                'ticker': 'NAVER_WORLD_SOX',
            },
...
def fetch_naver_finance_themes_and_sectors():
  """네이버 증권 테마 및 업종 페이지에서 실시간 주도 테마 및 관련 종목 정보를 스크래핑합니다."""
  ...
  try:
    ...
    with urllib.request.urlopen(req, context=get_ssl_context(), timeout=2.0) as response:
      html_content = response.read().decode("euc-kr", errors="ignore")
      soup = BeautifulSoup(html_content, "html.parser")

      # 네이버 증권 테마 테이블 파싱
      for row in soup.select("table.type_5 tr"):
        col_name = row.select_one("td.col_1 a")
        col_rate = row.select_one("td.col_3 span")
        if col_name:
          theme_name = col_name.text.strip()
          theme_rate = col_rate.text.strip() if col_rate else "+0.00%"
          # 상승세인 테마 우선 수집 필터링
          is_up = not theme_rate.startswith("-")
          extracted_themes.append({
              "name": theme_name,
              "rate": theme_rate,
              "is_up": is_up,
          })
          if len(extracted_themes) >= 8:
            break
  except Exception:
    pass

  # 만약 크롤링 실패 시 대체 기본 테마 데이터 제공
  if not extracted_themes:
    extracted_themes = [
        {"name": "반도체 장비/재료", "rate": "+2.45%", "is_up": True},
        {"name": "전력기기/인프라", "rate": "+1.85%", "is_up": True},
        {"name": "바이오시밀러", "rate": "+0.95%", "is_up": True},
        {"name": "조선/방산", "rate": "+1.20%", "is_up": True},
        {"name": "저PBR(은행/금융)", "rate": "+0.60%", "is_up": True},
    ]
...
def fetch_yahoo_data(ticker):
  ...
  try:
    ...
    with urllib.request.urlopen(
        req, context=get_ssl_context(), timeout=0.8
    ) as response:
...
def fetch_realtime_data(ticker):
  ...
  try:
    ...
    if ticker.startswith("NAVER_DOMESTIC_"):
      ...
      api_url = (
          f"https://polling.finance.naver.com/api/realtime/domestic/index/{target}"
      )
    elif ticker.startswith("NAVER_WORLD_SPOT_"):
      spot_map = {"SP": ".INX", "DOW": ".DJI", "NAS": ".IXIC"}
      symbol = spot_map.get(
          ticker.replace("NAVER_WORLD_SPOT_", ""), ".IXIC"
      )
    ...
    if api_url:
      ...
      with urllib.request.urlopen(
        ...
        if isinstance(res_json, dict):
          if (
              "closePrice" in res_json
              or "price" in res_json
              or "nowValue" in res_json
              or "dealBasRate" in res_json
          ):
            item = res_json
        ...
        if not item and isinstance(res_json, list) and len(res_json) > 0:
          item = (
              res_json["datas"][0]
              if isinstance(res_json, dict) and "datas" in res_json
              else res_json[0]
          )
        ...
        if item:
          cur_price = (
              item.get("closePrice")
              or item.get("nowValue")
              or item.get("price")
              or item.get("dealBasRate")
          )
          fluc_rate = (
              item.get("fluctuationsRatio")
              or item.get("rate")
              or item.get("fluctuationRate")
              or 0
          )
          ...
          if cur_price is not None:
            ...
            rate_val = (
                float(str(fluc_rate).replace("%", "").replace("+", ""))
                if fluc_rate
                else 0.0
            )
            is_up = not (
                sign in ["4", "5"] or str(fluc_rate).startswith("-")
            )
            return {
...
def get_all_quotes_cached():
  ...
  with ThreadPoolExecutor(max_workers=15) as executor:
    future_to_code = {
        executor.submit(fetch_realtime_data, ticker): code
    ...
    for future in as_completed(future_to_code):
      ...
      try:
        ...
        price_map[code] = (
            data
            if data
            else {"price": "일시적 지연", "rate": "+0.00%", "is_up": True}
        )
      except Exception:
        price_map[code] = {
            "price": "일시적 지연",
...
def fetch_feature_stocks():
  ...
  try:
    ...
    with urllib.request.urlopen(
        req, context=get_ssl_context(), timeout=2.0
    ) as response:
  ...
  if is_market_closed:
    market_summary_keyword = (
        ...
        "• [순환매 전개]: 단기 자금이 **반도체 대형주(삼성전자·SK하이닉스)**에서"
        " **저PBR 금융주(KB금융·신한지주)** 및 **전력기기 섹터**로 순환 이동\n"
        "• [향후 전망]: 글로벌 매크로 지표 및 야간 선물 시장 연동성 검토"
    )
  else:
    market_summary_keyword = (
        "• [수급 동향]: **AI 반도체 및 핵심 소부장** 중심의 선별적 매수세 유입\n"
        "• [순환매 전개]: 초반 2차전지 및 바이오 섹터로 유입되던 자금이 오후장"
        " 들어 **전력기기·방산 섹터 및 저PBR 금융주**로 빠르게 순환 이동\n"
...
def fetch_naver_finance_news():
  ...
  query_str = urllib.parse.quote(
      "연합인포맥스 OR 뉴스핌 OR 금리 OR 환율 OR 실적 OR 영업이익 OR 외국인 OR 수급 OR"
      " FOMC OR 증시 when:6h"
  )
  ...
  try:
    ...
    with urllib.request.urlopen(
        req, context=get_ssl_context(), timeout=1.5
    ) as response:
      xml_data = response.read()
      root = ET.fromstring(xml_data)
      ...
      for item in root.findall(".//item"):
        ...
        rss_source_name = (
            item.find("source").text if item.find("source") is not None else ""
        )
        ...
        for existing_title in collected_titles:
          similarity = SequenceMatcher(
              None, title_clean, existing_title
          ).ratio()
          if similarity >= 0.75:
            is_duplicate = True
            break
        ...
        if is_negative:
          ...
          comment = "관련 이슈에 따른 단기 변동성 확대 및 리스크 관리 주의"
            ...
            k in title_clean for k in ["실적", "서프라이즈", "영업이익", "수주", "계약"]
        ):
          news_type = "호재"
          comment = (
              "실적 개선 및 모멘텀 유입에 따른 긍정적 주가 영향 기대"
          )
...
def generate_theme_sync_analysis(quotes, news_list):
  sox = quotes.get(
      "phlx", {"price": "-", "rate": "+0.00%", "is_up": True}
  )
  nasdaq_fut = quotes.get(
      "nasdaq_fut", {"price": "-", "rate": "+0.00%", "is_up": True}
  )
  ...
  if is_up:
    us_driver = (
        "글로벌 빅테크 반도체 밸류체인 연동 강세: 필라델피아"
        f" 반도체(**{sox_rate}**) 및 나스닥 선물(**{nasdaq_rate}**)의 우상향"
        " 흐름은 국내 반도체 수출 실적 개선 기대감을 선반영하며 지수"
        " 상단을 지지하고 있습니다."
    )
    core_stocks = "**NVIDIA, 마이크론, ASML**"
    domestic_stocks = "**삼성전자, SK하이닉스**"
    risk_strategy = (
        "실적 모멘텀이 검증된 펀더멘털 우량주 중심의 공격적 비중 확대 및"
        " 눌림목 트레이딩"
    )
  else:
    us_driver = (
        "글로벌 기술주 멀티플 조정 압력: 필라델피아 반도체(**{sox_rate}**)"
        f" 조정 및 나스닥 선물(**{nasdaq_rate}**)의 경계감 반영은 국내 증시의"
        " 단기 변동성을 확대시키는 주요 요인으로 작용합니다."
    )
    core_stocks = "**테슬라, 애플, 마이크로소프트**"
    domestic_stocks = "**KB금융, 현대차, 삼성바이오로직스**"
    risk_strategy = (
        "매크로 변동성 심화 국면에서 펀더멘털이 탄탄한 방어적 포트폴리오 구축"
        " 및 리스크 관리"
    )
...
def generate_smart_money_analysis(quotes, newspim_news):
  kospi = quotes.get(
      "kospi", {"price": "0", "rate": "+0.00%", "is_up": True}
  )
  kosdaq = quotes.get(
      "kosdaq", {"price": "0", "rate": "+0.00%", "is_up": True}
  )
  usdkrw = quotes.get(
      "usdkrw", {"price": "1,300", "rate": "+0.00%", "is_up": True}
  )
  ...
  if newspim_news:
    newspim_snippet = (
        f' (뉴스핌 실시간 보도 참고: "**{newspim_news[0]}**")'
    )
  ...
  if kospi_up:
    ...
    domestic_text = (
        "국내 현·선물 수급 동향: 코스피(**"
        f"{kospi_rate}**) 및 코스닥(**{kosdaq_rate}**)의 상승 탄력과 함께"
        f" 외국인·기관의 우호적 수급 유입이 포착됩니다{newspim_snippet}."
    )
  else:
    ...
    domestic_text = (
        "국내 현·선물 수급 동향: 코스피(**"
        f"{kospi_rate}**) 및 코스닥(**{kosdaq_rate}**) 하락 압력 속에서"
        f" 기관·외인 매물 출회 및 보수적 대응이 우세합니다{newspim_snippet}."
    )

  if kospi_up and not kosdaq.get("is_up", True):
    decoupling_text = (
        "코스피 대형주 중심의 자금 집중 현상과 코스닥 개별주 조정 간의"
        " 디커플링 장세가 진행 중입니다."
    )
  elif not kospi_up and kosdaq.get("is_up", True):
    decoupling_text = (
        "코스피가 조정을 받는 동안 코스닥 중소형주로 개인 및 단기"
        " 스마트머니의 순환매가 유입되는 양상입니다."
    )
  else:
    direction_word = "동반 강세" if kospi_up else "동반 약세"
    decoupling_text = (
        f"양시장 모두 **{direction_word}** 흐름을 보이며 지수 연동성이"
        " 높게 유지되고 있습니다."
    )
  ...
  if kospi_up:
    concentrated_themes = (
        ...
        " 1) <strong>AI 반도체 및 핵심 소부장(삼성전자·SK하이닉스)</strong>"
        " 중심의 이익 성장 동반 구조적 쏠림 현상이 지속되며, 2) 변동성 장세"
        " 속 수익성 방어를 위한 <strong>전력기기·원전·조선</strong> 및"
        " <strong>은행·보험 등 저PBR 주주환원 업종</strong>으로 자금이 분산·확산되는"
        " 순환매 흐름이 포착됩니다."
    )
  else:
    concentrated_themes = (
        "<strong>현재 스마트머니 수급 집중 테마 및 업종 분석:</strong>"
        " 1) 지수 방어 및 변동성 회피를 위한 <strong>저PBR 금융주(KB금융·신한지주) 및"
        " 통신·유틸리티</strong> 방어주로 피난처 성격의 자금이 유입되며, 2)"
        " 개별 모멘텀을 보유한 일부 테마주 중심으로만 단기 트레이딩 자금이"
        " 순환하고 있습니다."
    )
  ...
  if usdkrw.get("is_up", True):
    fx_oil_text = (
        f"원/달러 환율(**{fx_price}원**, **{fx_rate}**) 상승 압력에 따른 외국인"
        " 수급 이탈 우려 점검 및 변동성 주의"
    )
  else:
    fx_oil_text = (
        f"원/달러 환율(**{fx_price}원**, **{fx_rate}**) 하향 안정세에 힘입어"
        " 외국인 수급 유입 환경 개선 모니터링"
    )
  ...
  return [
      {
          ...
          "desc": (
              "글로벌 AI 인프라 투자 확대에 따른 실적 턴어라운드 종목 집중"
              " 공략"
          ),
          "stock": "**삼성전자, SK하이닉스, 한미반도체** - AI 반도체",
          "rank": "TOP 1",
      },
      {
          ...
          "stock": "**HD현대일렉트릭, 효성중공업** - 전력기기",
          "rank": "TOP 2",
      },
      {
          "title": "바이오 CDMO 실적 우량주 및 파이프라인 모멘텀",
          "desc": "어닝 개선 기대감 및 스마트머니 수급 유입 포착",
          "stock": "**삼성바이오로직스, 셀트리온** - 바이오",
          "rank": "TOP 3",
      },
      {
          "title": "K-방산 및 조선 슈퍼사이클 실적 턴어라운드",
          "desc": (
              "환율 효과 및 인도 기준 실적 성장이 담보된 수주형 성장주"
          ),
          "stock": "**한화에어로스페이스, 현대로템** - 방산",
          "rank": "TOP 4",
      },
      {
...
def generate_premarket_summary_bullets(quotes, news_list):
  ...
  header_title = (
      f"{today_str}, 장 시작 전 뉴욕증시 마감 및 매크로 핵심 브리핑"
  )
  ...
  if us_macro_news:
    bullets.append(
        "[글로벌 마감 핵심] 간밤 뉴욕증시와 연계된 주요 매크로 이슈로"
        f' "**{us_macro_news[0]}**"(이)가 시장의 주요 변동성 요인으로'
        " 작용했습니다."
    )
  else:
    bullets.append(
        "[글로벌 마감 핵심] 뉴욕증시 주요 지수 혼조세 속 연준 정책 및 금리"
        " 동향에 따른 투자 심리가 교차하고 있습니다."
    )

  if macro_indicator_news:
    bullets.append(
        "[경제 지표 점검] 실시간 주요 경제 동향으로"
        f' "**{macro_indicator_news[0]}**" 관련 소식이 전해지며, 글로벌'
        " 통화정책 압력을 가중시키고 있습니다."
    )
  else:
    bullets.append(
        "[경제 지표 점검] 다가오는 주요 경제 지표 발표 및 연준 주요 인사의"
        " 통화정책 발언에 따른 글로벌 금리 변동성을 모니터링해야 합니다."
    )

  if is_up:
    bullets.append(
        "[국내 증시 영향] 간밤 해외 지수 및 선물 강세"
        f" (**{nasdaq_rate}**)의 영향으로, 오늘 국내 증시는 **AI 반도체 및"
        " 전력기기** 업종을 중심으로 탄력적인 매수세 유입이 예상됩니다."
    )
    market_summary_tags = [
        "**삼성전자**",
        "**SK하이닉스**",
    ...
    bullets.append(
        "[국내 증시 영향] 간밤 뉴욕증시 조정 및 야간 선물 약세"
        f" (**{nasdaq_rate}**)의 여파로, 오늘 국내 증시는 **고베타 업종**을"
        " 중심으로 매물 출회 및 변동성 확대가 예상됩니다."
    )
    market_summary_tags = [
        "**KB금융**",
        "**신한지주** (저PBR 금융주)",
  ...
  strategy_text = (
      "지수 상승 탄력과 수급 유입에 발맞춰, 핵심 주도 테마 내 실적 우량"
      " 종목의 지지선 확인 후 분할 매수 및 순환매 대응이 유효합니다."
      if is_up
      else "지수 하방 압력에 대응하여 방어적 포트폴리오를 구성하고 무리한"
      " 추격 매수를 자제하는 보수적 관점 유지가 안전합니다."
  )
...
def generate_ai_comprehensive_briefing(quotes, news_list):
  ...
  return (
      ...
      f"📊 [시황 총평]\n"
      f"나스닥 선물(**{nasdaq_fut['rate']}**)과 필라델피아 반도체 지수"
      f"(**{sox['rate']}**) 변동성을 소화하며 대형주 중심의 완만한 수급 균형이"
      f" 나타나고 있습니다. 원/달러 환율(**{usdkrw['price']}원**) 추이에"
      " 주목합니다.\n\n"
      f"🔍 [핵심 체크포인트]\n"
      f'• 주요 헤드라인: "**{top_news}**"\n'
      "• 코스피·코스닥 거래대금 유입 및 주도 섹터 순환매 속도 확인\n\n"
      f"💡 [실전 대응 가이드]\n"
      "• 지수 변동성 구간에서는 수급이 집중되는 핵심 주도주 눌림목 위주로"
      " 대응\n"
      "• 매크로 리스크 방어를 위한 실적 우량주 분산 병행"
  )
...
def index():
  ...
  # 네이버 증권 브리핑 연동을 위한 실시간 테마/업종 수집
  naver_themes = fetch_naver_finance_themes_and_sectors()
  ...
  market_summary_header, market_summary_bullets, market_summary_tags = (
      generate_premarket_summary_bullets(price_map, live_news)
  )
  ...
  return render_template(
      ...
      naver_themes=naver_themes,  # 브리핑 연동 네이버 증권 테마/업종 정보 추가
  )
...
def api_ai_briefing():
  ...
  ai_briefing_text = generate_ai_comprehensive_briefing(
      price_map, news_list=news_list
  )
